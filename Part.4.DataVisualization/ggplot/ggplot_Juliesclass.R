...
####### Edited by Avery Grant #######
########### 10 March 2023 ###########

#This is a follow along for DataVis-ggplot-primer located:
#https://github.com/tparchman/GAIN_summer2022/tree/main/day2_datavis
...
unique(city_df$State)

#basic ggplot layout 
ggplot(data=city_df,aes(x=Time,y=Growth))

#don't need to specify data, x, y, etc.
ggplot(city_df,aes(Time,Growth))
...
### BUT WHY IS THE COLOR AES NOT IN GEOM_POINT()?! ###

ggplot(data=city_df,aes(x=Time,y=Growth)) + 
  geom_point(aes(color = State))

####
#Be careful of too many categories
ggplot(data=city_df,aes(x=Time,y=Growth,color=City)) + 
  geom_point()
...
#remove steps with geom_path
ggplot(data=city_df,aes(x=Time,y=Growth,color=State)) + 
  geom_path() 
#can also just use geom_line
...
ggplot(data=city_df,aes(x=Time,y=Growth,colour=State)) + 
  geom_point() + 
  stat_smooth()
#can use geom_smooth instead of stat_smooth


#if we didnt want to add a line for each state but wanted one overall trendline, 
...
#make trendline look a little nicer
ggplot(data=city_df,aes(x=Time,y=Growth)) + 
  geom_point(aes(colour=State),size=2) + 
  stat_smooth(linetype='dashed',colour='black',linewidth=1)

#add shape for population size
ggplot(data=city_df,aes(x=Time,y=Growth)) + 
  geom_point(aes(colour=State,shape=Size),size=4) + 
  stat_smooth(linetype='dashed',colour='black',linewidth=1, se = FALSE)
...
ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  geom_point(aes(colour=State,shape=Size),size=4) + 
  stat_smooth(method='glm',linetype='dashed',colour='black',linewidth=2)
...
#Looking at summarise first 
city_sum_df_subset <- city_df %>%
  group_by(State) %>%
  summarise(Temp_mean = mean(Temp, na.rm = TRUE), #temp mean
            ...
            Ppt_n = n())
view(city_sum_df_subset)

#Now let's add mutate
city_sum_df <- city_df %>%
  group_by(State) %>%
  summarise(Temp_mean = mean(Temp, na.rm = TRUE), #temp mean
            Temp_sd = sd(Temp, na.rm = TRUE), #temp standard deviation
...
#info on calculating confidence interval from standard error
#browseURL("https://doi.org/10.1136/bmj.331.7521.903")
...
#unstack them with geom_bar(position)
ggplot(data=city_sum_df,aes(x=State,y=mean,fill=Clim)) +
...
ggplot(data=city_sum_df,aes(x=State,y=mean,fill=Clim)) +
  facet_wrap(~Clim,nrow=1) + 
  geom_bar(stat='identity')

#can't interpret temp data because its on the same Y scale as Ppt
#Not a useful plot

#Freeing up the scales!
...
ggplot(data=city_sum_df,aes(x=State,y=mean,fill=Clim)) +
  facet_wrap(~Clim,nrow=2,scales = 'free') + 
  geom_bar(stat = 'identity')
...
ggplot(data=city_sum_df,aes(x=State,y=mean,colour=Clim)) +
  facet_wrap(~Clim,nrow=1,scales = 'free') + 
  geom_errorbar(aes(ymin = mean - ci, ymax = mean + ci),colour='purple') +
  geom_point(size=5)
...
view(city_long_df)
#now we will have 2 rows per city: separated into ppt and temp by column clim
#city_long_df[1:10, ]

#make bar and error but using stat_summary()
ggplot(data = city_long_df, aes(x = State, y = value, fill = Clim)) +
  facet_wrap(~Clim, nrow = 1, scales = "free") + 
  stat_summary(fun = mean,geom = "bar") + 
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar",color = "black")

#make point and error bar but using stat_summary()
...
ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  geom_point(aes(colour=State_name),size=4) + 
  stat_smooth(method='lm',linetype='dashed',colour='black',size=2) + 
  xlab('Temperature (c)') + ylab('Precipitation (in)') + 
  ggtitle(label='Temp vs. Precip.', subtitle = 'By State')
####see subtitle
...
ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  geom_point(aes(fill=State_name),size=4,pch=24,colour='black') + 
  stat_smooth(method='lm',linetype='dashed',colour='darkgrey',size=2,se=FALSE) + 
  xlab('Temperature (c)') + ylab('Precipitation (in)') + 
...
ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  ...
  stat_smooth(method='lm',linetype='dashed',colour='black',size=2) + 
  ggtitle('Temp vs. Precip. by State') + 
  scale_fill_manual(name='State:',values=c('mistyrose2','cadetblue3','goldenrod1','palegreen2','mediumpurple3')) + 
  scale_shape_manual(name='State:',values=c(21,22,23,24,25)) + 
  scale_x_continuous(name='Temperature (c)',limits=c(5,25),breaks=c(5,10,15,20,25),position = 'top') + 
  scale_y_continuous(name='Precipitation (in)',limits=c(0,1500),breaks=c(0,250,500,1000,1250,1500)) 

#color PDF
browseURL('https://www.nceas.ucsb.edu/sites/default/files/2020-04/colorPaletteCheatsheet.pdf')

######################################################################
#### Theme ####
######################################################################

#changes figure layout
...
ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  ...
  scale_fill_manual(name='State:',values=c('mistyrose2','cadetblue3','goldenrod1','palegreen2','mediumpurple3')) + 
  scale_shape_manual(name='State:',values=c(21,22,23,24,25)) + 
  scale_x_continuous(name='Temperature (c)',limits=c(5,25),breaks=c(5,10,15,20,25)) + 
  scale_y_continuous(name='Precipitation (in)',limits=c(0,1500),breaks=c(0,250,500,1000,1250,1500)) + 
  theme_bw()
...
#### SO MANY OPTIONS ####
#this will change as you find what you like and need for each figure
#here is just one Trevor commonly starts with then edit as needed 

ggplot(data=city_df,aes(x=Temp,y=Ppt)) + 
  geom_point(aes(fill=State_name,shape=State_name),size=4,colour='black') + 
  stat_smooth(method='lm',linetype='dashed',colour='black',linewidth=2) + 
  ggtitle('Temp vs. Precip. by State') + #titles always kinda look bad
  scale_fill_frontiers(name='State:') + #adding colors -- journal and tv show colors
  scale_shape_manual(name='State:',values=c(21,22,23,24,25)) + 
  scale_x_continuous(name='Temperature (c)') + 
  scale_y_continuous(name='Precipitation (in)',limits=c(0,1500),breaks=c(0,250,500,750,1000,1250,1500)) + 
  theme_bw() +
  theme(legend.position = c(.8, .8), #choose exactly where you want your legend
...
# ggarrange in library(ggpubr) is a good package to use
...
(worst_plot + very_bad_plot) / (bad_plot + okay_plot)
...
ppt_plot <- ggplot(data=ppt_df,aes(x=State,y=value,fill=State)) +
  ...
  scale_fill_npg() + 
  ggtitle('Precip best') +
  scale_y_continuous(name='Precipitation (in)',position='right',breaks=c(250,500,750,1000,12)) + 
  theme_bw() +
  theme(legend.position = 'None', 
        plot.title = element_text(size = 26, colour="black",face = "bold"),
        axis.text = element_text(size=18),
...
#### compare against the worst plot ####
